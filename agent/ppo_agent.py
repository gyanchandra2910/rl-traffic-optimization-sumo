"""
Proximal Policy Optimization (PPO) Agent for Traffic Light Control.

Context in Project: "Traffic Signal Optimization using RL"
Implements the PPO algorithm featuring a clipped surrogate objective.
This is a highly advanced Policy Gradient architecture that provides
immense training stability.
"""
...
try:
    ...
    print(f"[WARNING] PyTorch unavailable ({exc}). PPO agent will not be available.")
...
if TORCH_AVAILABLE:
    class ActorCriticNet(nn.Module):
        ...
        Because our state space is highly compact (6D Macroscopic), we use
        a lightweight 2-hidden-layer network (64 units). This ensures the
        model trains blisteringly fast on a CPU without overfitting.
        """
        def __init__(self, state_dim: int = 6, action_dim: int = 5):
            super(ActorCriticNet, self).__init__()
            # Actor outputs action probabilities (logits)
            ...
            # Critic evaluates the state's expected return
...
class PPOAgent:
    ...
    Mathematical Intuition (Why PPO?):
    Standard Policy Gradients can take destructively large step sizes, ruining
    the learned policy. PPO fixes this via a "Clipped Surrogate Objective". 
    By clipping the probability ratio (r_t) of the new policy vs the old policy
    between [1-epsilon, 1+epsilon], PPO guarantees monotonic, safe improvements.

    State Normalization:
    Deep RL networks suffer from "Gradient Explosions" if input values are too
    large. In traffic, Wait Times can routinely exceed 1000+, while Phase Indices
    are just 0-3. We use Welford's online running mean/variance algorithm to 
    robustly normalize all inputs to a mean of 0 and std of 1.
    """
    def __init__(self, state_dim: int = 6, action_dim: int = 5,
                 lr: float = 0.0003, gamma: float = 0.95,
                 batch_size: int = 360, ppo_epochs: int = 4,
                 clip_param: float = 0.2, entropy_coef: float = 0.01,
                 **kwargs):
        """
        Initialize the PPO agent parameters.

        Args:
            state_dim (int): Observation space size.
            action_dim (int): Action space size.
            batch_size (int): Experiences collected before a PPO update phase.
            ppo_epochs (int): Number of optimization passes over the collected batch.
            clip_param (float): The epsilon clipping bound (typically 0.2).
        """
    ...
    def learn(self, state: np.ndarray, action: int, reward: float, next_state: np.ndarray):
        """
        Store transition and trigger a PPO update if the batch is full.
        """
        # Normalize without double counting (mean/var already updated in choose_action)
    ...
    def update(self):
        """
        Execute the PPO Clipped Surrogate Optimization process.
        """
        ...
        with torch.no_grad():
            ...
            # Advantage Normalization (another crucial stabilization trick)
        ...
        for _ in range(self.ppo_epochs):
            ...
            # Actor Loss uses minimum of unclipped and clipped
            actor_loss = -torch.min(surr1, surr2).mean()
            # Critic Loss minimizes TD Error
            critic_loss = nn.MSELoss()(values.squeeze(1), td_targets)
            
            # Total objective (incorporating entropy for exploration)
    ...
    def decay_epsilon(self):
        """PPO is strictly stochastic, no epsilon required."""
        pass
